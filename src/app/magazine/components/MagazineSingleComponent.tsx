...
const MagazineSingleComponent: React.FC<Props> = ({ magazine }) => {
  ...
  const links = [
    { name: 'Download .PDF', link: magazine.downloadPdf },
    {
      name: 'Download .ePUB',
      link: magazine.downloadEpub,
    },
  ];
  return (
      ...
      <div className="flex justify-between gap-13">
        <div className="flex flex-col gap-6">
          ...
          <div className="flex flex-row gap-3.5">
            {links.map((link) => (
              <ButtonRegular
                className="border-borders h-[61px] border-1 bg-transparent"
                key={link.link}
              >
                <div className="font-inter text-gray flex gap-3.5 text-xl font-medium">
                  {link.name}
                  <Image
